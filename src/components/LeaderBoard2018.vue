      ...
      >
        <template slot-scope="props">
          ...
          <b-table-column label="Total Loss">
            <span v-if="props.row.total_change < 0" class="success">{{ props.row.total_change }} lbs</span>
...
data.forEach(p => {
  ...
  contestant.total_change =
    Math.round((p.weighins[currentWeek] - p.weighins[0]) * 10) / 10;
  contestant.percent_change = Math.abs(
    Math.round(((p.weighins[currentWeek] / p.weighins[0]) * 100 - 100) * 100) /
      100
  );
