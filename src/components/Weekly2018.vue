          ...
          >
            <template slot-scope="props">
              ...
              <b-table-column
                ...
                numeric
              >{{ props.row.start_weight }}</b-table-column>
              <b-table-column
                label="End Weight"
                style="min-width:120px;"
                numeric
              >{{ props.row.end_weight }}</b-table-column>
              <b-table-column label="Total Loss" style="min-width:120px;" numeric>
                <span
                  v-if="props.row.total_change < 0"
                  class="success"
                >{{ props.row.total_change }} lbs</span>
                <span v-else>+{{ props.row.total_change }} lbs</span>
              </b-table-column>
              <b-table-column
...
for (var i = 1; i <= 12; i++) {
  ...
  data.forEach(p => {
    ...
    contestant.total_change =
      Math.round((p.weighins[i] - p.weighins[i - 1]) * 10) / 10;
    contestant.percent_change = Math.abs(
      Math.round(((p.weighins[i] / p.weighins[i - 1]) * 100 - 100) * 100) / 100
    );
